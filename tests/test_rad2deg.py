...
def test_rad2deg(
    ...
    return Payload(
        lambda input, out: _rad2deg(input, out, implementation_index),
        _torch_rad2deg,
        (input, out),
        {},
        rtol=rtol,
...
def _rad2deg(input, out, implementation_index):
    infini.ops.rad2deg(
        input,
        out,
...
def _torch_rad2deg(input, out):
    out.copy_(torch.rad2deg(input))
