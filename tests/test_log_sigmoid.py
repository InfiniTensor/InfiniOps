...
def test_log_sigmoid(
    ...
    return Payload(
        lambda input, out: _log_sigmoid(input, out, implementation_index),
        _torch_log_sigmoid,
        (input, out),
        {},
        rtol=rtol,
...
def _log_sigmoid(input, out, implementation_index):
    infini.ops.log_sigmoid(
        input,
        out,
...
def _torch_log_sigmoid(input, out):
    out.copy_(F.logsigmoid(input))
