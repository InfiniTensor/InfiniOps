...
def test_square(
    ...
    return Payload(
        lambda input, out: _square(input, out, implementation_index),
        _torch_square,
        (input, out),
        {},
        rtol=rtol,
...
def _square(input, out, implementation_index):
    infini.ops.square(
        input,
        out,
...
def _torch_square(input, out):
    out.copy_(torch.square(input))
