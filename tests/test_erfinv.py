...
def test_erfinv(
    ...
    return Payload(
        lambda input, out: _erfinv(input, out, implementation_index),
        _torch_erfinv,
        (input, out),
        {},
        rtol=rtol,
...
def _erfinv(input, out, implementation_index):
    infini.ops.erfinv(
        input,
        out,
...
def _torch_erfinv(input, out):
    out.copy_(torch.erfinv(input))
