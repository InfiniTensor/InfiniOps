...
def test_arccos(
    ...
    return Payload(
        lambda input, out: _arccos(input, out, implementation_index),
        _torch_arccos,
        (input, out),
        {},
        rtol=rtol,
...
def _arccos(input, out, implementation_index):
    infini.ops.arccos(
        input,
        out,
...
def _torch_arccos(input, out):
    out.copy_(torch.arccos(input))
