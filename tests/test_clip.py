...
def test_clip(
    ...
    return Payload(
        lambda input, out: _clip(input, out, implementation_index),
        _torch_clip,
        (input, out),
        {},
        rtol=rtol,
...
def _clip(input, out, implementation_index):
    infini.ops.clip(
        input,
        -0.5,
        0.5,
...
def _torch_clip(input, out):
    out.copy_(torch.clip(input, min=-0.5, max=0.5))
