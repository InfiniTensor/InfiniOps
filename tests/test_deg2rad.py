...
def test_deg2rad(
    ...
    return Payload(
        lambda input, out: _deg2rad(input, out, implementation_index),
        _torch_deg2rad,
        (input, out),
        {},
        rtol=rtol,
...
def _deg2rad(input, out, implementation_index):
    infini.ops.deg2rad(
        input,
        out,
...
def _torch_deg2rad(input, out):
    out.copy_(torch.deg2rad(input))
