...
def test_hardshrink(
    ...
    return Payload(
        lambda input, out: _hardshrink(input, out, implementation_index),
        _torch_hardshrink,
        (input, out),
        {},
        rtol=rtol,
...
def _hardshrink(input, out, implementation_index):
    infini.ops.hardshrink(
        input,
        0.5,
        out,
...
def _torch_hardshrink(input, out):
    out.copy_(F.hardshrink(input, lambd=0.5))
