...
def test_addcdiv(
    ...
    if device == "cpu" and implementation_index == 8 and dtype == torch.float16:
        pytest.skip("ATen `addcdiv.out` does not support float16 on CPU")
    ...
    tensor2 = tensor2.sign() * tensor2.abs().clamp_min(0.5)
    out = empty_strided(shape, out_strides, dtype=dtype, device=device)

    return Payload(
        lambda input, tensor1, tensor2, out: _addcdiv(
...
def _addcdiv(input, tensor1, tensor2, out, implementation_index):
    infini.ops.addcdiv(
        input,
        tensor1,
        tensor2,
...
def _torch_addcdiv(input, tensor1, tensor2, out):
    out.copy_(torch.addcdiv(input, tensor1, tensor2, value=0.5))
