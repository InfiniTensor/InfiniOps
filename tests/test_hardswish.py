...
def test_hardswish(
    ...
    return Payload(
        lambda input, out: _hardswish(input, out, implementation_index),
        _torch_hardswish,
        (input, out),
        {},
        rtol=rtol,
...
def _hardswish(input, out, implementation_index):
    infini.ops.hardswish(
        input,
        out,
...
def _torch_hardswish(input, out):
    out.copy_(F.hardswish(input))
