...
def test_negative(
    ...
    return Payload(
        lambda input, out: _negative(input, out, implementation_index),
        _torch_negative,
        (input, out),
        {},
        rtol=rtol,
...
def _negative(input, out, implementation_index):
    infini.ops.negative(
        input,
        out,
...
def _torch_negative(input, out):
    out.copy_(torch.negative(input))
