...
def test_arcsinh(
    ...
    return Payload(
        lambda input, out: _arcsinh(input, out, implementation_index),
        _torch_arcsinh,
        (input, out),
        {},
        rtol=rtol,
...
def _arcsinh(input, out, implementation_index):
    infini.ops.arcsinh(
        input,
        out,
...
def _torch_arcsinh(input, out):
    out.copy_(torch.arcsinh(input))
