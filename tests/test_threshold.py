...
def test_threshold(
    ...
    return Payload(
        lambda input, out: _threshold(input, out, implementation_index),
        _torch_threshold,
        (input, out),
        {},
        rtol=rtol,
...
def _threshold(input, out, implementation_index):
    infini.ops.threshold(
        input,
        0.25,
        -1.0,
        out,
...
def _torch_threshold(input, out):
    out.copy_(F.threshold(input, threshold=0.25, value=-1.0))
