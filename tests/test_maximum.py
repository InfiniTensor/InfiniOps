...
@pytest.mark.parametrize(
    "shape, input_strides, other_strides, out_strides", _SHAPE_CASES
)
...
def test_maximum(
    ...
    return Payload(
        lambda input, other, out: _maximum(input, other, out, implementation_index),
        _torch_maximum,
        (input, other, out),
        {},
        rtol=rtol,
...
def _maximum(input, other, out, implementation_index):
    infini.ops.maximum(
        input,
        other,
        out,
...
def _torch_maximum(input, other, out):
    out.copy_(torch.maximum(input, other))
