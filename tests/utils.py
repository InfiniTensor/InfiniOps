...
def get_npu_stream(tensor):
    """Backward-compat wrapper used by tests extracted from `feat/ascend-operators`.

    Delegates to `get_stream(device)` which prefers the device-specific
    `current_stream` API over `torch.accelerator.current_stream`.
    """
    return get_stream(tensor.device)
