...
def test_leaky_relu(
    ...
    return Payload(
        lambda input, out: _leaky_relu(input, out, implementation_index),
        _torch_leaky_relu,
        (input, out),
        {},
        rtol=rtol,
...
def _leaky_relu(input, out, implementation_index):
    infini.ops.leaky_relu(
        input,
        0.125,
        out,
...
def _torch_leaky_relu(input, out):
    out.copy_(F.leaky_relu(input, negative_slope=0.125))
