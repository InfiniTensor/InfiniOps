...
def test_arcsin(
    ...
    return Payload(
        lambda input, out: _arcsin(input, out, implementation_index),
        _torch_arcsin,
        (input, out),
        {},
        rtol=rtol,
...
def _arcsin(input, out, implementation_index):
    infini.ops.arcsin(
        input,
        out,
...
def _torch_arcsin(input, out):
    out.copy_(torch.arcsin(input))
