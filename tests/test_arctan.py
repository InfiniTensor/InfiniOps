...
def test_arctan(
    ...
    return Payload(
        lambda input, out: _arctan(input, out, implementation_index),
        _torch_arctan,
        (input, out),
        {},
        rtol=rtol,
...
def _arctan(input, out, implementation_index):
    infini.ops.arctan(
        input,
        out,
...
def _torch_arctan(input, out):
    out.copy_(torch.arctan(input))
