...
@pytest.mark.parametrize("device", ("cuda",))
def test_flash_attention_prefill_multi_cuda(
    num_heads,
    num_kv_heads,
    head_size,
    dtype,
    ...
    """Multi-sequence prefill with cu_seqlens on CUDA."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
...
@pytest.mark.parametrize("device", ("cuda",))
def test_flash_attention_paged_decode_cuda(
    num_heads,
    num_kv_heads,
    head_size,
    block_size,
    ...
    """Decode phase: single token per request with paged KV cache on CUDA."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
    ...
    cu_seqlens_q = torch.arange(0, num_reqs + 1, dtype=torch.int64, device=device)
    cu_seqlens_kv = torch.tensor(
        [i * kv_len for i in range(num_reqs + 1)],
        dtype=torch.int64,
        device=device,
    )
