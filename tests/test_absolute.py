...
def test_absolute(
    ...
    return Payload(
        lambda input, out: _absolute(input, out, implementation_index),
        _torch_absolute,
        (input, out),
        {},
        rtol=rtol,
...
def _absolute(input, out, implementation_index):
    infini.ops.absolute(
        input,
        out,
...
def _torch_absolute(input, out):
    out.copy_(torch.absolute(input))
