...
def test_sigmoid(
    ...
    return Payload(
        lambda input, out: _sigmoid(input, out, implementation_index),
        _torch_sigmoid,
        (input, out),
        {},
        rtol=rtol,
...
def _sigmoid(input, out, implementation_index):
    infini.ops.sigmoid(
        input,
        out,
...
def _torch_sigmoid(input, out):
    out.copy_(torch.sigmoid(input))
