...
def test_softplus(
    ...
    return Payload(
        lambda input, out: _softplus(input, out, implementation_index),
        _torch_softplus,
        (input, out),
        {},
        rtol=rtol,
...
def _softplus(input, out, implementation_index):
    infini.ops.softplus(
        input,
        1.0,
        20.0,
...
def _torch_softplus(input, out):
    out.copy_(F.softplus(input, beta=1.0, threshold=20.0))
