...
def test_operator_selection_tracks_implementation_header_changes():
    ...
    for root in ("native", "torch", "ninetoothed", "linked"):
...
def test_select_ops_from_config_filters_implementation_slots(tmp_path):
    ...
    assert module._select_ops_from_config(
        {"add": [slot_0, slot_16]}, config, "ops.json"
    ) == {"add": [slot_16]}
...
def test_select_ops_from_config_rejects_missing_slot(tmp_path):
    ...
    try:
        module._select_ops_from_config({"add": [slot_0]}, config, "ops.json")
    except ValueError as exc:
...
def test_select_ops_from_config_preserves_legacy_headers():
    module = _load_generator_module()
    config = {
        "add": {
            "headers": ["src/native/cpu/ops/add/add.h"],
