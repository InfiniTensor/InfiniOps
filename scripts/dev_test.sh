...
usage() {
    cat <<'EOF'
Usage:
  scripts/dev_test.sh [cpu|nvidia|iluvatar|hygon|metax|moore|cambricon|ascend|auto] [--op-category CATEGORY] [--report PATH] [--no-build] [-- pytest-args...]

Examples:
  scripts/dev_test.sh cambricon
  scripts/dev_test.sh metax
  scripts/dev_test.sh nvidia
  scripts/dev_test.sh nvidia --op-category linalg
  scripts/dev_test.sh nvidia --op-category comparison-logic-bitwise
...
Notes:
  ...
  - `--op-category` may be repeated; accepted slugs are:
    basic-math, comparison-logic-bitwise, reduction-stat-sort, linalg, nn,
    tensor-organization, fft-special.
...
if [[ ${#op_categories[@]} -gt 0 ]]; then
    op_category_suffix="$(
        PYTHONPATH="${repo_root}${PYTHONPATH:+:${PYTHONPATH}}" \
            python3 -m scripts.operator_categories --slug "${op_categories[@]}"
    )"

    for category in "${op_categories[@]}"; do
        op_category_args+=(--op-category "$category")
    done
fi
...
if [[ -z "$report_path" ]]; then
    if [[ -n "$op_category_suffix" && "$op_category_suffix" != "all" ]]; then
        report_path="${repo_root}/reports/${platform}-${op_category_suffix}.json"
    else
...
if [[ ${#op_categories[@]} -gt 0 ]]; then
    echo "[dev_test] category : ${op_categories[*]}"
fi
...
PYTHONPATH="${install_dir}${PYTHONPATH:+:${PYTHONPATH}}" \
    python3 -m pytest "${pytest_targets[@]}" --devices "${platform}" --op-report "${report_path}" "${op_category_args[@]}" "${pytest_args[@]}"
pytest_status=$?
set -e
