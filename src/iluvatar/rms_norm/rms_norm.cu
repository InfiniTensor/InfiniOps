#ifndef INFINI_OPS_ILUVATAR_RMS_NORM_CU_
#define INFINI_OPS_ILUVATAR_RMS_NORM_CU_
...
#include "data_type.h"
#include "iluvatar/rms_norm/rms_norm.h"
...
void Operator<RmsNorm, Device::Type::kIluvatar>::operator()(
    void* stream, Tensor y, const Tensor x, const Tensor w,
    float /*epsilon*/) const {
  cudaSetDevice(y.device().index());
  auto cuda_stream = static_cast<cudaStream_t>(stream ? stream : 0);
