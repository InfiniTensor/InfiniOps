#ifndef INFINI_OPS_BASE_THRESHOLD_BACKWARD_GRAD_INPUT_H_
#define INFINI_OPS_BASE_THRESHOLD_BACKWARD_GRAD_INPUT_H_

#include "operator.h"

namespace infini::ops {

class ThresholdBackwardGradInput : public Operator<ThresholdBackwardGradInput> {
 public:
  ThresholdBackwardGradInput(const Tensor grad_output, const Tensor self,
                             const double threshold, Tensor grad_input)
  ...
  virtual void operator()(const Tensor grad_output, const Tensor self,
                          const double threshold, Tensor grad_input) const = 0;
