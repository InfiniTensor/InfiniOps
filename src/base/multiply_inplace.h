#ifndef INFINI_OPS_BASE_MULTIPLY_INPLACE_H_
#define INFINI_OPS_BASE_MULTIPLY_INPLACE_H_

#include "operator.h"

namespace infini::ops {

class MultiplyInplace : public Operator<MultiplyInplace> {
 public:
  MultiplyInplace(Tensor input, const Tensor other)
  ...
  MultiplyInplace(Tensor input, const double other)
      : input_shape_{input.shape()},
        input_strides_{input.strides()},
        input_type_{input.dtype()},
        other_{other},
