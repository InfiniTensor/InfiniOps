#ifndef INFINI_OPS_BASE_MASKED_FILL_INPLACE_H_
#define INFINI_OPS_BASE_MASKED_FILL_INPLACE_H_

#include "operator.h"

namespace infini::ops {

class MaskedFillInplace : public Operator<MaskedFillInplace> {
 public:
  MaskedFillInplace(Tensor input, const Tensor mask, const double value)
  ...
  MaskedFillInplace(Tensor input, const Tensor mask, const Tensor value)
      : input_shape_{input.shape()},
        input_strides_{input.strides()},
        input_type_{input.dtype()},
        mask_shape_{mask.shape()},
