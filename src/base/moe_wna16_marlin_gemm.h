...
class MoeWna16MarlinGemm : public Operator<MoeWna16MarlinGemm> {
 public:
  MoeWna16MarlinGemm(
      ...
      std::optional<Tensor> global_scale, std::optional<Tensor> b_zeros_or_none,
      ...
      const int64_t top_k, const bool mul_topk_weights, const bool is_ep,
      const int64_t b_q_type_id, const int64_t size_m, const int64_t size_n,
      const int64_t size_k, const bool is_full_k, const bool use_atomic_add,
      const bool use_fp32_reduce, const bool is_zp_float, Tensor out)
      : a_metadata_{a},
    ...
    Validate(a, b_q_weight, b_scales, global_scale, b_zeros_or_none,
             g_idx_or_none, perm_or_none, workspace, sorted_token_ids,
             expert_ids, num_tokens_past_padded, topk_weights, out);
  }
 ...
 protected:
  void ValidateCallMetadata(
      ...
      const int64_t b_q_type_id, const int64_t size_m, const int64_t size_n,
      const int64_t size_k, const bool is_full_k, const bool use_atomic_add,
      const bool use_fp32_reduce, const bool is_zp_float, Tensor out) const {
    assert(moe_block_size == moe_block_size_ && top_k == top_k_ &&
           mul_topk_weights == mul_topk_weights_ && is_ep == is_ep_ &&
           b_q_type_id == b_q_type_id_ && size_m == size_m_ &&
           size_n == size_n_ && size_k == size_k_ && is_full_k == is_full_k_ &&
 ...
 private:
  void Validate(const Tensor a, const Tensor b_q_weight, const Tensor b_scales,
    ...
    assert(
        ...
        a.IsContiguous() &&
        "`MoeWna16MarlinGemm` requires contiguous float16 or bfloat16 "
        "`a`");
    ...
    const auto supported_qtype =
        b_q_type_id_ == kUint4B8 || b_q_type_id_ == kUint8B128 ||
        b_q_type_id_ == kUint4 || b_q_type_id_ == kUint8 ||
        b_q_type_id_ == kFloat8E4M3Fn;
    ...
    const auto pack_factor =
        (b_q_type_id_ == kUint8B128 || b_q_type_id_ == kUint8 ||
         b_q_type_id_ == kFloat8E4M3Fn)
            ? 4
            : 8;
    ...
    assert(b_scales.ndim() == 3 && b_scales.size(0) == b_q_weight.size(0) &&
           ...
           size_k_ % b_scales.size(1) == 0 && b_scales.dtype() == a.dtype() &&
    ...
    if (b_zeros_or_none) {
      assert(b_zeros_or_none->ndim() == 3 &&
             ...
             (is_zp_float_ ? b_zeros_or_none->dtype() == a.dtype()
    ...
    assert(topk_weights.shape() ==
               Tensor::Shape({static_cast<Tensor::Size>(size_m_),
                              static_cast<Tensor::Size>(top_k_)}) &&
    ...
    assert(out.shape() ==
           ...
           out.dtype() == a.dtype() && out.IsContiguous() &&
