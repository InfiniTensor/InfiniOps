#ifndef INFINI_OPS_BASE_MULTIPLY_H_
#define INFINI_OPS_BASE_MULTIPLY_H_

#include "operator.h"

namespace infini::ops {

class Multiply : public Operator<Multiply> {
 public:
  Multiply(const Tensor input, const Tensor other, Tensor out)
  ...
  Multiply(Tensor input, const Tensor other)
      : input_shape_{input.shape()},
        input_strides_{input.strides()},
        input_type_{input.dtype()},
        other_shape_{other.shape()},
  ...
  Multiply(const Tensor input, const double other, Tensor out)
      : input_shape_{input.shape()},
        input_strides_{input.strides()},
        input_type_{input.dtype()},
        out_shape_{out.shape()},
