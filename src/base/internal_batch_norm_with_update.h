#ifndef INFINI_OPS_BASE_INTERNAL_BATCH_NORM_WITH_UPDATE_H_
#define INFINI_OPS_BASE_INTERNAL_BATCH_NORM_WITH_UPDATE_H_
...
class BatchNormWithUpdate : public Operator<BatchNormWithUpdate> {
 public:
  BatchNormWithUpdate(const Tensor input, const std::optional<Tensor> weight,
                      const std::optional<Tensor> bias, Tensor running_mean,
                      Tensor running_var, const double momentum,
                      const double eps, Tensor out, Tensor save_mean,
                      Tensor save_invstd, Tensor reserve)
  ...
  virtual void operator()(const Tensor input,
                          ...
                          Tensor running_var, const double momentum,
                          const double eps, Tensor out, Tensor save_mean,
                          Tensor save_invstd, Tensor reserve) const = 0;
