#ifndef INFINI_OPS_BASE_SET_INPLACE_H_
#define INFINI_OPS_BASE_SET_INPLACE_H_
...
class SetInplace : public Operator<SetInplace> {
 public:
  SetInplace(Tensor input, const Tensor source, const int64_t storage_offset,
  ...
  SetInplace(Tensor input, const Tensor source)
      : input_shape_{input.shape()},
        input_strides_{input.strides()},
        input_type_{input.dtype()},
        source_shape_{source.shape()},
