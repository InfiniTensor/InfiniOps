#ifndef INFINI_OPS_BASE_PREPARE_MOE_INPUT_H_
#define INFINI_OPS_BASE_PREPARE_MOE_INPUT_H_
...
// Prepare routing metadata for SGLang's CUTLASS MoE path.
class PrepareMoeInput : public Operator<PrepareMoeInput> {
 public:
  PrepareMoeInput(const Tensor topk_ids, const int64_t num_experts,
 ...
 protected:
  void ValidateCallMetadata(
      const Tensor topk_ids, const int64_t num_experts, const int64_t n,
      const int64_t k, const Tensor expert_offsets, const Tensor problem_sizes1,
      const Tensor problem_sizes2, const Tensor input_permutation,
    ...
    assert(CallMetadataMatches(topk_ids, expert_offsets, problem_sizes1,
           ...
           "`PrepareMoeInput` tensor metadata differs from its descriptor");
  }
  ...
  void Validate(const Tensor topk_ids, const Tensor expert_offsets,
                ...
                const Tensor input_permutation, const Tensor output_permutation,
                const std::optional<Tensor> blockscale_offsets) const {
    assert(topk_ids.ndim() == 2 && "`PrepareMoeInput` requires 2D `topk_ids`");
    assert(topk_ids.dtype() == DataType::kInt32 &&
           "`PrepareMoeInput` requires int32 `topk_ids`");
    assert(topk_ids.IsContiguous() &&
           "`PrepareMoeInput` requires contiguous `topk_ids`");
    assert(topk_ > 0 && "`PrepareMoeInput` requires a positive top-k size");
    assert(num_experts_ > 0 &&
           num_experts_ <= std::numeric_limits<int32_t>::max() / 3 &&
           "`PrepareMoeInput` requires indexable int32 `num_experts`");
    assert(n_ > 0 && n_ <= std::numeric_limits<int32_t>::max() / 2 && k_ > 0 &&
           k_ <= std::numeric_limits<int32_t>::max() &&
           "`PrepareMoeInput` requires positive int32 GEMM dimensions");
    assert(numel_ <=
               static_cast<Tensor::Size>(std::numeric_limits<int32_t>::max()) &&
           "`PrepareMoeInput` requires int32-addressable routing indices");
    if (blockscale_offsets) {
      ...
      assert(max_blockscale_padding <= std::numeric_limits<int32_t>::max() &&
             ...
             "`PrepareMoeInput` blockscale offsets must fit int32");
    }
    ...
    assert(
        ...
        "`PrepareMoeInput` requires all tensors on the same device");
    ...
    assert(expert_offsets.ndim() == 1 &&
           ...
           "`PrepareMoeInput` requires `expert_offsets` shape "
           "[`num_experts + 1`]");
    ...
    assert(is_int32_contiguous(expert_offsets) &&
           ...
           "`PrepareMoeInput` requires contiguous int32 outputs");
  }
...
#endif  // INFINI_OPS_BASE_PREPARE_MOE_INPUT_H_
