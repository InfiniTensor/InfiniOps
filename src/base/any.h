#ifndef INFINI_OPS_BASE_ANY_H_
#define INFINI_OPS_BASE_ANY_H_
...
class Any : public Operator<Any> {
 public:
  Any(const Tensor input, const int64_t dim, const bool keepdim, Tensor out)
  ...
  Any(const Tensor input, const std::optional<std::vector<int64_t>> dim,
  ...
  Any(const Tensor input, Tensor out)
      : input_shape_{input.shape()},
        input_strides_{input.strides()},
        input_type_{input.dtype()},
        out_shape_{out.shape()},
