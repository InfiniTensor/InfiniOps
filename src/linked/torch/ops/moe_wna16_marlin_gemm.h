...
class TorchMoeWna16MarlinGemm : public ::infini::ops::MoeWna16MarlinGemm {
 public:
  ...
  void operator()(const Tensor a, const Tensor b_q_weight,
                  const Tensor b_scales, std::optional<Tensor> global_scale,
                  ...
                  const bool is_ep, const int64_t b_q_type_id,
                  const int64_t size_m, const int64_t size_n,
                  const int64_t size_k, const bool is_full_k,
                  const bool use_atomic_add, const bool use_fp32_reduce,
                  const bool is_zp_float, Tensor out) const override {
    ValidateCallMetadata(a, b_q_weight, b_scales, global_scale, b_zeros_or_none,
                         ...
                         is_ep, b_q_type_id, size_m, size_n, size_k, is_full_k,
                         use_atomic_add, use_fp32_reduce, is_zp_float, out);

    Backend::Validate(b_q_type_id, global_scale.has_value());

    const typename Backend::StreamGuard stream_guard{
        Backend::GetStreamFromExternal(stream_, device_index_)};
    Backend::Call(ToAten(a), ToAten(out), ToAten(b_q_weight), ToAten(b_scales),
                  ToOptionalAten(global_scale), ToOptionalAten(b_zeros_or_none),
