...
TorchSiluAndMul<kDev, Provider>::TorchSiluAndMul(const Tensor input, Tensor out)
    : ::infini::ops::SiluAndMul{input, out},
      device_index_{out.device().index()} {}

template <Device::Type kDev, typename Provider>
void TorchSiluAndMul<kDev, Provider>::operator()(const Tensor input,
                                                 Tensor out) const {
  const TorchContextGuard<kDev> context_guard{stream_, device_index_};
  auto at_input =
  ...
  Provider::Call(provider_out, provider_input);
