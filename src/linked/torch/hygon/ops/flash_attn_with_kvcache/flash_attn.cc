#include "linked/torch/hygon/ops/flash_attn_with_kvcache/flash_attn.h"

#include "linked/torch/ops/flash_attn_with_kvcache.h"
#include "torch/hygon/c10.h"

extern "C" std::vector<at::Tensor> mha_fwd_kvcache(
    ...
    int num_splits, const std::optional<at::Tensor>& s_aux);

namespace infini::ops::linked::torch::hygon {

struct FlashAttnKvcache : C10<Device::Type::kHygon> {
...
std::vector<at::Tensor> FlashAttnKvcache::Call(
  ...
  return ::mha_fwd_kvcache(q, k_cache, v_cache, k, v, cache_seqlens, rotary_cos,
                           rotary_sin, cache_batch_idx, cache_leftpad,
                           block_table, alibi_slopes, out, softmax_scale,
                           causal, window_size_left, window_size_right, softcap,
                           rotary_interleaved, num_splits, s_aux);
}
...
template class TorchFlashAttnWithKvcache<
    ::infini::ops::linked::torch::hygon::FlashAttnKvcache>;

}  // namespace infini::ops::linked::torch

namespace infini::ops {

void Operator<FlashAttnWithKvcache, Device::Type::kHygon, 16>::operator()(
  ...
  using Delegate = linked::torch::TorchFlashAttnWithKvcache<
      linked::torch::hygon::FlashAttnKvcache>;
  if (!delegate_) {
