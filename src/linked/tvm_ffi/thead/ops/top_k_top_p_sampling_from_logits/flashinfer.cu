...
#include "linked/tvm_ffi/thead/ops/top_k_top_p_sampling_from_logits/flashinfer.h"
...
__global__ void CastLogits(float* dst, const Src* src, std::size_t count) {
  for (auto index =
    ...
    dst[index] = Caster<Device::Type::kThead>::Cast<float>(src[index]);
  }
}
...
__global__ void GatherCastLogits(float* dst, const Src* src,
  ...
  for (auto index =
    ...
    dst[index] = Caster<Device::Type::kThead>::Cast<float>(
...
void CallTopP(DLTensor* probs, DLTensor* output, DLTensor* indices,
              DLTensor* top_p, bool deterministic, uint64_t seed,
              uint64_t offset) {
  tvm::ffi::Function::InvokeExternC(
      nullptr, __tvm_ffi_top_p_sampling_from_probs, tvm::ffi::TensorView(probs),
      tvm::ffi::TensorView(output), OptionalView(indices),
      OptionalTensorView{tvm::ffi::TensorView(top_p)}, 1.0, deterministic, seed,
      offset);
}

void CallJoint(DLTensor* probs, DLTensor* output, DLTensor* indices,
               DLTensor* top_k, DLTensor* top_p, bool deterministic,
               uint64_t seed, uint64_t offset) {
  tvm::ffi::Function::InvokeExternC(
      nullptr, __tvm_ffi_top_k_top_p_sampling_from_probs,
      tvm::ffi::TensorView(probs), tvm::ffi::TensorView(output),
      OptionalView(indices), OptionalTensorView{tvm::ffi::TensorView(top_k)}, 0.0,
...
void Validate(const Tensor logits, const Tensor top_k, const Tensor top_p,
  ...
  Require(logits.device().type() == Device::Type::kThead &&
              out.device() == logits.device() && logits.IsContiguous() &&
              out.IsContiguous(),
          "FlashInferSampling requires contiguous Thead logits and output");
  ...
  if (indices) {
    Require((indices->device() == logits.device() ||
            ...
            "FlashInferSampling requires contiguous CPU or Thead indices "
            "matching output");
  } else {
...
Operator<TopKTopPSamplingFromLogits, Device::Type::kThead, 16>::Operator(
...
void Operator<TopKTopPSamplingFromLogits, Device::Type::kThead,
...
std::size_t Operator<TopKTopPSamplingFromLogits, Device::Type::kThead,
...
void Operator<TopKTopPSamplingFromLogits, Device::Type::kThead,
  ...
  Require(logits.ndim() == 2 && logits.size(0) == logits_batch_size_ &&
              ...
              logits.device().type() == Device::Type::kThead &&
  ...
  DispatchFunc<Device::Type::kThead, AllFloatTypes>(
      logits.dtype(),
      [&](auto tag) {
  ...
  if (filter_apply_order == "top_k_first") {
    ...
    CallTopP(&matrix_a_tensor, &output_tensor, nullptr, &top_p_tensor,
             deterministic, actual_seed, actual_offset);
  } else {
    CallSoftmax(&scratch_tensor, &matrix_a_tensor, &matrix_b_tensor);
    CallJoint(&matrix_b_tensor, &output_tensor, nullptr, &top_k_tensor,
              &top_p_tensor, deterministic, actual_seed,
              actual_offset);
  }
