...
class KernelRmsNorm {
 public:
    ...
    __aicore__ inline void Init(
        ...
        xGm.SetGlobalBuffer(
            (__gm__ T *)x + rowOffset * dimLengthAlign,
 ...
 private:
    __aicore__ inline void CopyIn(int64_t row) {
        ...
        AscendC::DataCopyPad(
            xLocal, xGm[row * this->dimLengthAlign], params, pad);
    ...
    __aicore__ inline void Compute(int64_t row) {
        ...
        if constexpr (sizeof(T) == 4) {
            ...
            AscendC::LocalTensor<float> xF32 =
                xFp32Buf.Get<float>();
            AscendC::LocalTensor<float> tmpF32 =
                tmpFp32Buf.Get<float>();

            // Cast input fp16 → fp32.
            AscendC::Cast(xF32, xLocal,
...
extern "C" __global__ __aicore__ void rms_norm(
    ...
    if (dtypeSize == 2) {
        ...
        op.Init(x, weight, y, totalRows, dimLength, dimLengthAlign,
