 ...
 public:
    ...
    __aicore__ inline void Init(
        ...
        if (blockIdx < formerNum) {
            ...
            rowOffset =
                formerLength * formerNum + tailLength * tailIdx;
        }

        // Global memory pointers.
        x1Gm.SetGlobalBuffer(
            (__gm__ T *)x1 + rowOffset * dimLengthAlign,
            this->blockRows * dimLengthAlign);
        x2Gm.SetGlobalBuffer(
            (__gm__ T *)x2 + rowOffset * dimLengthAlign,
            this->blockRows * dimLengthAlign);
        yGm.SetGlobalBuffer(
            (__gm__ T *)y + rowOffset * dimLengthAlign,
            this->blockRows * dimLengthAlign);
        xOutGm.SetGlobalBuffer(
            (__gm__ T *)x_out + rowOffset * dimLengthAlign,
            this->blockRows * dimLengthAlign);
        weightGm.SetGlobalBuffer(
            (__gm__ float *)weight, dimLengthAlign);

        int32_t dimLenAlign =
            static_cast<int32_t>(this->dimLengthAlign);
        ...
        AscendC::DataCopyExtParams wParams{
            1,
            static_cast<uint32_t>(dimLenAlign * sizeof(float)),
            0, 0, 0};
 ...
 private:
    __aicore__ inline void CopyIn(int64_t row) {
        ...
        AscendC::DataCopyExtParams params{
            1,
            static_cast<uint32_t>(this->dimLengthAlign * sizeof(T)),
            0, 0, 0};
        AscendC::DataCopyPadExtParams<T> pad{
            false, 0, 0, static_cast<T>(0)};
        AscendC::DataCopyPad(
            x1Local, x1Gm[row * this->dimLengthAlign], params, pad);
        AscendC::DataCopyPad(
            x2Local, x2Gm[row * this->dimLengthAlign], params, pad);
    ...
    __aicore__ inline void Compute(int64_t row) {
        ...
        AscendC::LocalTensor<T> xOutLocal =
            outQueueXOut.AllocTensor<T>();
        ...
        int32_t dimLen =
            static_cast<int32_t>(this->dimLength);
        int32_t dimLenAlign =
            static_cast<int32_t>(this->dimLengthAlign);
        ...
        if constexpr (sizeof(T) == 4) {
            ...
            float meanVal =
                sumVal / static_cast<float>(dimLen) + this->eps;
            ...
            AscendC::Cast(b1, x1Local,
                          AscendC::RoundMode::CAST_NONE, dimLenAlign);
            AscendC::Cast(b2, x2Local,
                          AscendC::RoundMode::CAST_NONE, dimLenAlign);
            ...
            AscendC::Cast(xOutLocal, b1,
                          AscendC::RoundMode::CAST_ROUND, dimLenAlign);
    ...
    __aicore__ inline void CopyOut(int64_t row) {
        ...
        AscendC::DataCopyExtParams params{
            ...
            0, 0, 0};
        AscendC::DataCopyPad(
            yGm[row * this->dimLengthAlign], yLocal, params);
        AscendC::DataCopyPad(
            xOutGm[row * this->dimLengthAlign], xOutLocal, params);
...
extern "C" __global__ __aicore__ void add_rms_norm(
    ...
    if (dtypeSize == 2) {
        ...
        op.Init(x1, x2, weight, y, x_out, totalRows, dimLength,
                dimLengthAlign, formerNum, formerLength, tailLength, eps);
        op.Process();
    } else {
