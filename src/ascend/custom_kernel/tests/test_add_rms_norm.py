...
def test_add_rms_norm_correctness(dtype, shape):
    ...
    # Run CPU reference.
    y_ref, x_out_ref = _ref_add_rms_norm(
        x1.cpu(), x2.cpu(), weight.cpu(), eps
    )
    ...
    assert torch.allclose(
        x_out_npu.cpu(), x_out_ref, rtol=rtol_xout, atol=atol_xout
    ), (
        f"x_out mismatch: max_diff="
    ...
    assert torch.allclose(y_npu.cpu(), y_ref, rtol=rtol, atol=atol), (
        f"y mismatch: max_diff="
        f"{(y_npu.cpu() - y_ref).abs().max().item()}"
    )
