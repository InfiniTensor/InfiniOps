...
class Operator<RmsNorm, Device::Type::kCambricon> : public RmsNorm {
 public:
  Operator(const Tensor input, const Tensor weight, float eps, Tensor out): RmsNorm{input, weight, eps, out} {
    cnrt_utils::GetLaunchConfig(input.device(), &core_per_cluster, &cluster_count);
  ...
  void operator()(const Tensor input, const Tensor weight, float eps,
    ...
    DispatchFunc<DataType::kFloat16, DataType::kBFloat16, DataType::kFloat32>(
        input.dtype(),
        [&](auto input_tag) {
          using InputT = typename decltype(input_tag)::type;
          using CambriconInputT = TypeMapTypeDevice<DataTypeMapValue<InputT>, Device::Type::kCambricon>;

          DispatchFunc<DataType::kFloat16, DataType::kBFloat16,
                       DataType::kFloat32>(
              weight.dtype(),
              [&](auto weight_tag) {
                using WeightT = typename decltype(weight_tag)::type;
                using CambriconWeightT = TypeMapTypeDevice<DataTypeMapValue<WeightT>, Device::Type::kCambricon>;

                RmsnormUnion<CambriconInputT, CambriconWeightT>(
              ...
              "CambriconRmsNorm::operator() - weight dispatch");
        },
