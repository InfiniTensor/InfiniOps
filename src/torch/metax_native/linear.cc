...
void Operator<Linear, Device::Type::kMetax>::operator()(
    const Tensor a, const Tensor b, std::optional<Tensor> bias, bool trans_a,
    bool trans_b, Tensor out) const {
  auto at_a = metax_torch_fallback::ToAten(a);
  auto at_b = metax_torch_fallback::ToAten(b);
  auto at_bias = metax_torch_fallback::ToAten(bias);
