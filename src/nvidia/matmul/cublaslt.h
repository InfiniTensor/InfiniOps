#ifndef INFINI_OPS_NVIDIA_MATMUL_CUBLASLT_H_
#define INFINI_OPS_NVIDIA_MATMUL_CUBLASLT_H_
...
template <>
class Operator<Matmul, Device::Type::kNvidia, 0> : public Matmul {
 public:
  ...
  void operator()(const Tensor a, const Tensor b, Tensor c, bool trans_a,
    ...
    const auto op_a{GetOpA(trans_a, trans_b)};
    const auto op_b{GetOpB(trans_a, trans_b)};
    ...
    assert(status == CUBLAS_STATUS_SUCCESS && returned_results > 0 &&
           "failed to find a cuBLASLt matmul algorithm");

    const float alpha{1.0f};
    const float beta{0.0f};
    status = cublasLtMatmul(
        GetHandle(), op_desc, &alpha, a_ptr, a_layout, b_ptr, b_layout, &beta,
        c.data(), c_layout, c.data(), c_layout, &heuristic.algo, workspace_,
        workspace_size_in_bytes_,
        static_cast<Runtime<Device::Type::kNvidia>::Stream>(stream_));
    assert(status == CUBLAS_STATUS_SUCCESS && "cuBLASLt matmul launch failed");
