...
// Forward-declare the `aclrtlaunch_rms_norm` launch symbol defined by
...
//     `fp16` or 8 for `fp32`).
//   - Input, weight, and output must be contiguous.
//   - The custom kernel binary must be linked (`BUILD_ASCEND_CUSTOM=ON`).
//
// Use implementation index 0 for other valid layouts and `bf16`.
template <>
 ...
 public:
  Operator(const Tensor input, const Tensor weight, float eps, Tensor out)
    ...
    assert((weight_dtype_ == DataType::kFloat16 ||
           ...
           "`RmsNorm`: `weight` must be `fp16`, `bf16`, or `fp32`");
    ...
    if (weight_dtype_ != DataType::kFloat32) {
  ...
  void operator()(const Tensor input, const Tensor weight, float eps,
    ...
    aclrtlaunch_rms_norm(block_dim, stream, const_cast<void*>(input.data()),
                         ...
                         static_cast<int64_t>(dtype_ == DataType::kFloat16));
  }
