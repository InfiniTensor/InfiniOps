...
#include "native/cuda/nvidia/ops/prepare_moe_input/kernel.cuh"
#include "native/cuda/nvidia/ops/prepare_moe_input/kernel.h"
 ...
 public:
  explicit DeviceGuard(int device_index) {
    ...
    assert(status == cudaSuccess &&
           "`PrepareMoeInput` failed to query the current CUDA device");

    if (previous_device_ != device_index) {
      status = cudaSetDevice(device_index);
      assert(status == cudaSuccess &&
             "`PrepareMoeInput` failed to select the input CUDA device");
      restore_ = true;
    }
  }
  ...
  ~DeviceGuard() {
    if (restore_) {
      ...
      assert(status == cudaSuccess &&
             "`PrepareMoeInput` failed to restore the CUDA device");
    }
  }
...
void Operator<PrepareMoeInput, Device::Type::kNvidia, 0>::operator()(
  ...
  ValidateCallMetadata(topk_ids, num_experts, n, k, expert_offsets,
  ...
  prepare_moe_input_detail::PrepareMoeInputKernel<<<1, kThreads, 0, stream>>>(
  ...
  assert(status == cudaSuccess &&
         "`PrepareMoeInput` CUDA kernel launch failed");
}
