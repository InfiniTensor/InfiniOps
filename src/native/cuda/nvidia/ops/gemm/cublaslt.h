...
class Operator<Gemm, Device::Type::kNvidia, 1> : public Gemm {
 public:
  ...
  void operator()(const Tensor a, const Tensor b, std::optional<float> alpha,
    ...
    {
      INFINI_OPS_HOST_RANGE_SCOPE(HostRangeLayer::kBackendSubmit);
      ...
      status = cublasLtMatrixLayoutCreate(&c_layout, c_dtype, matmul_m,
                                          matmul_n, c_ld);
      assert(status == CUBLAS_STATUS_SUCCESS &&
      ...
      status = cublasLtMatmul(
          GetHandle(), op_desc, GetAlphaPtr(alpha_value), a_ptr, a_layout,
          b_ptr, b_layout, GetBetaPtr(beta_value), c.data(), c_layout, c.data(),
