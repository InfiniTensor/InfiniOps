...
2. **Platform implementation** in `src/<platform>/`: the class must inherit from the base (e.g. `class Blas : public Gemm`). See `src/cuda/gemm/blas.h` and `src/nvidia/gemm/cublas.h`.
3. **Tests** in `tests/`:
