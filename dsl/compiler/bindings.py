"""Generate pybind11 and C API bindings for InfiniOps operators."""
...
def generate_all_bindings(
    ...
    generated_src_dir = output_dir / "src"
    ...
    for op_name, impl_paths in ops.items():
        ...
        op_impl_names = impl_names.get(pascal_name, {})

        source_path = generated_src_dir / op_name
        header_name = f"{op_name}.h"
        bind_func_name = f"Bind{pascal_name}"

        (bindings_dir / header_name).write_text(
            _generate_pybind11(operator, op_impl_names)
        )

        legacy_c_source, legacy_c_header = _generate_legacy_c(operator, impl_paths)
        source_path.mkdir(exist_ok=True)
        (generated_src_dir / op_name / "operator.cc").write_text(legacy_c_source)
        (include_dir / header_name).write_text(legacy_c_header)
    ...
    op_includes = "\n".join(f'#include "{h}"' for h in header_paths)
    bind_func_calls = "\n".join(f"{f}(m);" for f in bind_func_names)

    (bindings_dir / "ops.cc").write_text(
